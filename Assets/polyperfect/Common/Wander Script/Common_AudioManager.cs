...
namespace Polyperfect.Common
{
    public class Common_AudioManager : MonoBehaviour
    {
        ...
        [SerializeField] private float _inaudibleDistance;
        ...
        public static void PlaySound(AudioClip clip, Vector3 pos, Transform listener, Transform source)
        {
            if (!instance)
            {
            ...
            float distanceToListener = Vector3.Distance(source.position, listener.position);

            if (distanceToListener > instance._inaudibleDistance)
            {
                return;
            }
